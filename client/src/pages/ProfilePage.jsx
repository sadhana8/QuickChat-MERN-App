       ...
       <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
        ...
        <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
          <input onChange={(e)=> setSelectedImg(e.target.files[0])} type="file"  id="avatar" accept='.png, .jpg, .jpeg' hidden />
            ...
            value={bio}
            placeholder="Bio" 
            className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-voilet-500 resize-none h-24'
            rows={4}
             ></textarea>
            <button 
            onClick={() => navigate('/chat')}
            type='submit'
            className='bg-linear-to-r from-purple-400 to-voilet-600 text-white p-2 rounded-full text-lg cursor-pointer'>
              Save 
            </button>
           </form>
